    ...
    const float cloudStretchRaw = 36.0;
#elif CLOUD_QUALITY == 3
    const float cloudStretchRaw = 42.0;
#endif
...
#if CLOUD_QUALITY > 1
    const float cloudNarrowness = 0.00016;
#else
    const float cloudNarrowness = 0.00010;
#endif
...
float GetCloudNoise(vec3 tracePos, int cloudAltitude, float lTracePosXZ, float cloudPlayerPosY) {
    ...
    float total = 0.0;
        ...
        float noiseMult = 0.80;
        wind *= 0.5;
    #elif CLOUD_QUALITY == 2 || !defined DEFERRED1
        int sampleCount = 8;
        float persistance = 0.50;
        float noiseMult = 0.90;
    #elif CLOUD_QUALITY == 3
        int sampleCount = 10;
        float persistance = 0.55;
        float noiseMult = 0.85;
    #endif
    ...
    return noise - (threshold * 0.3 + 0.28);
}
...
vec4 GetVolumetricClouds(int cloudAltitude, float distanceThreshold, inout float cloudLinearDepth, float skyFade, float skyMult0, vec3 cameraPos, vec3 nPlayerPos, float lViewPosM, float VdotS, float VdotU, float dither) {
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        if (cloudNoise > 0.00001) {
            ...
            float opacityFactor = 1.0 - exp(-cloudNoise * 14.0); // Beer's law approximation
            float powderFactor = 1.0 - exp(-cloudNoise * 4.0); // Powder effect for volumetric depth

            float cloudShading = 1.0 - (higherPlaneAltitude - tracePos.y) / cloudTallness;
            cloudShading = pow(cloudShading, 1.5); // Smoother vertical gradient

            // Realistic Dual Henyey-Greenstein scattering
            float g1 = 0.85; // Forward scattering lobe
            float g2 = -0.3; // Backward scattering lobe
            float phase1 = (1.0 - g1*g1) / pow(1.0 + g1*g1 - 2.0*g1*VdotSM1, 1.5);
            float phase2 = (1.0 - g2*g2) / pow(1.0 + g2*g2 - 2.0*g2*VdotSM1, 1.5);
            float phase = mix(phase1, phase2, 0.4);
            float scattering = phase * (1.0 - opacityFactor * 0.8) * powderFactor;

            cloudShading *= 1.0 + 0.3 * VdotSM3 * (1.0 - opacityFactor) + VdotSM4 + scattering;

            vec3 colorSample = cloudAmbientColor * (0.20 + 0.80 * cloudShading) + cloudLightColor * cloudShading;
            colorSample += cloudLightColor * scattering;
            ...
            float cloudDistanceFactor = clamp(distanceRatio, 0.0, 0.85) * 1.176;
            float cloudFogFactor = pow2(pow1_5(clamp(distanceRatio, 0.0, 1.0)));
            float skyMult1 = 1.0 - 0.15 * (1.0 - skyFade) * max(sunVisibility2, nightFactor);
            float skyMult2 = 1.0 - 0.25 * skyFade;
            colorSample = mix(cloudSkyColor, colorSample * skyMult1, cloudFogFactor * skyMult2 * 0.78);
            ...
            volumetricClouds.a += opacityFactor * pow(cloudDistanceFactor, 0.4 + 8.0 * pow(abs(VdotSM1M), 90.0)) * cloudMult;
