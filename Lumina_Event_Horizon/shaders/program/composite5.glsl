...
//Pipeline Constants//
#define END_WHITE_HOLE_FLARE // [true false]
uniform sampler2D titlelogo;
...
void DoCompTonemap(inout vec3 color) {
    // Lottes tonemap modified for Lumina Shaders
...
void main() {
    ...
    float dither = texture2DLod(noisetex, texCoord * view / 128.0, 0.0).b;
        ...
        vec4 clipPosBH = gbufferProjection * vec4(mat3(gbufferModelView) * bhPosWorld, 1.0);
        vec3 screenPosBH = clipPosBH.xyz / clipPosBH.w * 0.5 + 0.5;
