...
#include "/lib/colors/lightAndAmbientColors.glsl"
    ...
    #include "/lib/atmospherics/eventHorizon.glsl"
#endif
...
void main() {
    ...
    if (z0 < 1.0) {
        ...
        #endif
        #ifdef END
            vec3 lensedDir = GetLensedDir(nViewPos, upVec, eastVec);
            vec3 lensedViewPos = mat3(gbufferModelView) * lensedDir;
            
            color.rgb = endSkyColor;
            
            if (length(lensedDir) > 0.5) { // Not inside event horizon
                color.rgb += GetEnderStars(lensedViewPos, VdotU);
            } else {
                color.rgb = vec3(0.0);
            }
