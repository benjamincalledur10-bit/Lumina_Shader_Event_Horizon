...
The current pre-release is **v1.3.9-rc.3**, targeting Minecraft **1.8 through 26.3**.
...
The 1.3.9-rc.3 compatibility changes have static validation; in-game verification across
...
2. Download the latest stable ZIP from an official source above. The pre-release
   test package is named `Lumina_Event_Horizon_v1.3.9-rc.3.zip`.
...
Adjust **Autofocus Blur Strength** and **Cinematic Blur Quality** to taste.
Cinematic quality (64 samples) is the default; Ultra (96 samples) is available
for smoother contours at a higher GPU cost. The quality control also applies to
Distance Blur and legacy Depth of Field. Filtering scales with render resolution.
The effect is off by default. Motion blur has been removed.
Visual verification in Minecraft is still pending.

## Cloud formation (development)

Clouds now use a shared 3D density field for the sky, reflections and ground
shadows, with open gaps between groups and softer opacity accumulation.
Adjust **Environment & Sky > Clouds > Lumina Cloud Shape > Cloud Separation**
to change spacing. Start at 1.0; larger values leave more open sky.
The new formation is awaiting visual validation in Minecraft.
