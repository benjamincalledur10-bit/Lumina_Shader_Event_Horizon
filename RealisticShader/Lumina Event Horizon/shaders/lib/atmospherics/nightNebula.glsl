...
const vec4 CLOUD1_COL = vec4(0.65, 0.35, 0.95, 0.6); // Deep Purple
const vec4 CLOUD2_COL = vec4(0.15, 0.55, 1.0, 0.5);  // Stellar Blue
const vec4 CLOUD3_COL = vec4(0.4, 0.95, 0.85, 1.0); // Neon Teal Highlight
...
vec3 GetNightNebula(vec3 viewPos, float VdotU, float VdotS) {
    ...
    nebulaTexture += fbmCloud2(zoomUV3, 0.22 + tide) * CLOUD1_COL;
    nebulaTexture += fbmCloud(zoomUV2 * 0.9, 0.30 - tide) * CLOUD2_COL;
    nebulaTexture = mix(nebulaTexture, CLOUD3_COL, fbmCloud(vec2(0.9 * zoomUV4.x, 0.9 * zoomUV4.y), 0.22 + tide2));
    ...
    return max(nebulaTexture.rgb * nebulaTexture.a * 1.6, vec3(0.0));
}