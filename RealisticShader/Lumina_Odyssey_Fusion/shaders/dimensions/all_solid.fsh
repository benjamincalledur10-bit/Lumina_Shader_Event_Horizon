...
void main() {
	vec4 lmtexcoord_local = lmtexcoord;
	vec4 color_local = color;
	...
	float torchlightmap = lmtexcoord_local.z;
	...
	float lightmap = clamp( (lmtexcoord_local.w-0.9) * 10.0,0.,1.);
	...
	vec2 adjustedTexCoord = lmtexcoord_local.xy;
	...
	if (dist < maxdist) {
		...
		float used_POM_DEPTH = 1.0;
 		...
 		if ( viewVector.z < 0.0 && depthmap < 0.9999 && depthmap > 0.00001) {	
			...
			// POM Self-Shadowing for LabPBR
			float pomShadow = 1.0;
			vec3 lightDir = normalize(tbnMatrix * mat3(gbufferModelViewInverse) * sunPosition);
			if (lightDir.z > 0.0) {
				vec3 shadowRay = coord;
				vec3 shadowStep = lightDir * 0.1 * POM_DEPTH;
				for (int i = 0; i < 8; i++) {
					shadowRay += shadowStep;
					if (shadowRay.p > 1.0 || shadowRay.s < 0.0 || shadowRay.s > 1.0 || shadowRay.t < 0.0 || shadowRay.t > 1.0) break;
					if (shadowRay.p < 1.0 - POM_DEPTH + POM_DEPTH * readNormal(shadowRay.st).a) {
						pomShadow = 0.0;
						break;
					}
				}
			}
			lmtexcoord_local.w *= mix(0.4, 1.0, pomShadow);
			color_local.rgb *= mix(0.4, 1.0, pomShadow);
		}
	}
#endif
	if(!ifPOM) adjustedTexCoord = lmtexcoord_local.xy;
	...
	vec4 Albedo = texture2D_POMSwitch(texture, adjustedTexCoord.xy, vec4(dcdx,dcdy), ifPOM, textureLOD) * color_local;
	...
	// #endif
		...
		// Special handling for grass block
			float strength = 1.0 - color_local.b;
	...
	if (Puddle_shape > 0.01 && rainStrength > 0.01) {
		float t = frameTimeCounter * 3.0;
		vec2 p = worldpos.xz * 3.0;
		vec2 f = fract(p);
		vec2 p0 = floor(p);
		...
		for(int i=-1; i<=1; i++) {
			for(int j=-1; j<=1; j++) {
				...
				vec3 p3 = fract((p0 + g).xyx * vec3(.1031, .1030, .0973));
				p3 += dot(p3, p3.yzx + 33.33);
				vec2 h = fract((p3.xx+p3.yz)*p3.zy);
				float time = fract(t + h.x);
				vec2 d = (g + h - f);
				float dist = length(d);
				float wave = sin((dist - time * 0.5) * 40.0) * exp(-time * 5.0) * smoothstep(0.5, 0.0, dist);
				if(time > 0.0 && dist < 0.5) {
					rippleNormal.x -= d.x * wave;
					rippleNormal.z -= d.y * wave;
				}
			}
		}
		rippleNormal = normalize(rippleNormal);
		vec3 viewRipple = mat3(gbufferModelView) * rippleNormal;
		normal = normalize(mix(normal, viewRipple, Puddle_shape * rainStrength * 0.6));
	}
	#endif
	...
	//////////////////////////////// 				//////////////////////////////// 
		...
		if (blockID == BLOCK_QUARTZ) {
			// Mirror-like finish for quartz
			SpecularTex.r = max(SpecularTex.r, 0.98); // High smoothness
			SpecularTex.g = max(SpecularTex.g, 0.85); // High reflectivity
		}

		SpecularTex.r = max(SpecularTex.r, max(rainfall, Puddle_shape * 0.95)); // Smoothness for puddles
		SpecularTex.g = max(SpecularTex.g, max(Puddle_shape * 0.9, 0.02)); // High reflectivity for puddles
	...
	//////////////////////////////// 				////////////////////////////////
		...
		vec2 PackLightmaps = vec2(torchlightmap, lmtexcoord_local.w);
