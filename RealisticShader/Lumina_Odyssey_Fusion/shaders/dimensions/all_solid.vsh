...
#ifdef ECLIPSE_GRASS
// Eclipse Grass: multi-axis procedural wind with randomized, organic motion
vec2 calcWave(in vec3 pos) {
    // Per-plant random phase offset based on world XZ to break uniformity
    float plantHash = fract(sin(dot(floor(pos.xz), vec2(12.9898, 78.233))) * 43758.5453);
    float phaseOffset = plantHash * 6.2831;

    // Three overlapping sine layers at different frequencies for fluid motion
    float time = pi2wt;
    float wave1 = sin(time * 0.0063 * 3.0 - pos.x * 0.7 + pos.z * 0.4 + phaseOffset) * 0.55;
    float wave2 = sin(time * 0.0042 * 3.0 + pos.z * 0.9 - pos.x * 0.3 + phaseOffset * 1.37) * 0.35;
    float wave3 = sin(time * 0.0098 * 3.0 + pos.x * 0.5 + pos.z * 0.6 + phaseOffset * 0.71) * 0.15;

    // Noise-driven magnitude variation so not all plants sway equally
    float magnitude = abs(sin(dot(vec4(frameTimeCounter, pos), vec4(1.0, 0.005, 0.005, 0.005))) * 0.5 + 0.72) * 0.016;
    magnitude *= (0.7 + plantHash * 0.6); // 0.7..1.3 range per plant

    vec2 ret = vec2(wave1 + wave2 * 0.7, wave2 + wave3 * 0.8) * magnitude;
    return ret;
}

vec3 calcMovePlants(in vec3 pos) {
    vec2 move1 = calcWave(pos);
    // Subtle vertical bob driven by horizontal displacement
    float move1y = -length(move1) * 0.8;
    return vec3(move1.x, move1y, move1.y) * 5.0 * WAVY_STRENGTH;
}
#else
// Original Bliss vegetation wave
vec2 calcWave(in vec3 pos) {
