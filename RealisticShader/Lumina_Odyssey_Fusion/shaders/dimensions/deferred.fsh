...
vec3 getAurora(vec3 viewVector) {
    if (isColdBiome < 0.01 || viewVector.y < 0.0) return vec3(0.0);
    
    float nightFade = clamp(-sunElevation * 2.0, 0.0, 1.0);
    if (nightFade < 0.01) return vec3(0.0);

    vec3 auroraCol = vec3(0.0);
    float time = frameTimeCounter * 0.01;
    
    vec2 p = viewVector.xz / max(viewVector.y, 0.01) * 0.2;
    
    for(int i = 0; i < 6; i++) {
        float h = float(i) * 0.15;
        vec2 p1 = p * (1.0 + h * 0.2) + vec2(time * 2.0, time * 0.5);
        vec2 p2 = p * (1.5 + h * 0.3) - vec2(time * 1.5, time * 1.2);
        
        float noise1 = auroraFBM(p1 * 0.03);
        float noise2 = auroraFBM(p2 * 0.04);
        
        float shape = sin(p.x * 2.0 + noise1 * 8.0) * cos(p.y * 2.0 + noise2 * 8.0);
        shape = smoothstep(0.4, 0.9, shape);
        
        vec3 color = mix(vec3(0.1, 1.0, 0.4), vec3(0.7, 0.1, 1.0), h);
        auroraCol += shape * color * (1.0 - h) * 0.6;
    }
    
    return auroraCol * nightFade * isColdBiome * (1.0 - rainStrength) * 4000.0;
}
// uniform float far;
