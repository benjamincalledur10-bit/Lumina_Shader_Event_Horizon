...
void main() {
  ...
  #ifdef NETHER_SHADER
    float luma = dot(texture2D(colortex7, texcoord).rgb, vec3(0.2126, 0.7152, 0.0722));
    float heatIntensity = clamp((luma - 0.5) * 1.5, 0.0, 1.0) + 0.05; // Base 0.05 everywhere, peaks at 1.0 near lava
    
    float hNoise = texture2D(noisetex, texcoord * 10.0 + vec2(0.0, frameTimeCounter * 0.2)).r;
    float hNoise2 = texture2D(noisetex, texcoord * 15.0 - vec2(0.0, frameTimeCounter * 0.15)).r;
    
    vec2 heatDistortion = vec2(hNoise - 0.5, hNoise2 - 0.5) * 0.01 * heatIntensity;
    texcoord += heatDistortion * (depth < 1.0 ? 1.0 : 0.0);
  #endif
