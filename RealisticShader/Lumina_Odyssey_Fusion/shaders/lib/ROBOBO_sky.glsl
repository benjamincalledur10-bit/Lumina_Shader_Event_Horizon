...
vec3 calculateAtmosphere(vec3 background, vec3 viewVector, vec3 upVector, vec3 sunVector, vec3 moonVector, out vec2 pid, out vec3 transmittance, const int iSteps, float noise) {
	...
	#ifdef SKY_GROUND
		float planetGround = exp(-20 * pow(max(-viewVector.y*2 + 0.05,0.0),2)); // Softer ground darkening
	#else
		float planetGround = 1.0; // Infinite horizon
	#endif
	...
	float high_sun = clamp(pow(sunVector.y+0.6,5),0.0,1.0) * 2.0; 
	float low_sun = clamp(((1.0-abs(sunVector.y))*2.) - high_sun,1.0,2.0) ;
	...
	for (int i = 0; i < iSteps; ++i, position += increment) {
		...
		#ifdef ORIGINAL_CHOCAPIC_SKY
			scatteringAmbient += sky_coefficientsScattering * stepAirmass.xy * stepScatteringVisible * planetGround;
		#else
			scatteringAmbient += sky_coefficientsScattering * stepAirmass.xy * stepScatteringVisible * low_sun * planetGround;
		#endif
