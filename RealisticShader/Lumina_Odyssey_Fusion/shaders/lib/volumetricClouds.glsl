...
float cloudVol(int layer, in vec3 pos, in vec3 samplePos, in float cov, in int LoD, float minHeight, float maxHeight){
	...
	float noise = 0.0 ;
	float amp = 1.0;
	float freq = mix(100.0, 200.0, upperPlane);
	float weightSum = 0.0;

	// Multi-layer FBM for high detail
	for (int i = 0; i < 3; i++) {
		noise += (1.0 - densityAtPos(samplePos * freq)) * amp;
		weightSum += amp;
		amp *= 0.5;
		freq *= 2.5;
	}
	...
	if (LoD > 0){
		float detail = abs( densityAtPos(samplePos * mix(450.0,600.0,upperPlane) ) - (1.0-clamp(((maxHeight - pos.y) / 100.0),0.0,1.0)));
		noise += detail * 0.75 * (1.0-cov);
	}

	noise = noise * noise;
	float cloud = max(cov - noise * noise * fbmAmount * 1.5, 0.0);
...
vec3 DoCloudLighting(
	...
	// Kappa-style strong internal scattering
	float powder = 1.0 - exp(-15.0 * density); // Strong powder effect for silver lining
	
	// Multiple scattering octaves for direct light
	vec3 directLight = sunScatter * exp(-8.0 * sunShadows);
	directLight += sunMultiScatter * exp(-2.0 * sunShadows) * 0.6 * powder;
	directLight += sunMultiScatter * exp(-0.5 * sunShadows) * 0.3 * powder;

	// Amplified ambient inside clouds for strong scattering look
	vec3 indirectLight = skyLightCol * mix(0.5, 1.5, exp(-2.0 * density));
	indirectLight *= pow(distantfog, 1.0 - rainStrength*0.5);
